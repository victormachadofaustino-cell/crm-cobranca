  ...
  return ( // -> Desenha a interface flutuante do modal na tela do usuário.
    ...
    <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(15, 23, 42, 0.4)", zIndex: 6000, display: "flex", justifyContent: "center", alignItems: "center" }}> {/* -> Cortina escura de fundo configurada com opacidade suave para isolar visualmente as planilhas de fundo. */}
      <div style={{ background: "#ffffff", padding: "24px", borderRadius: "8px", border: "1px solid #e2e8f0", width: "100%", maxWidth: "480px", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.05)", maxHeight: "85vh", overflowY: "auto", boxSizing: "border-box" }}> {/* -> Cartão branco otimizado com bordas executivas de 8px e trava antiqueda com rolagem vertical caso a tela seja pequena. */}
        ...
        {/* TOPO DO MODAL: TÍTULO INSTITUCIONAL JURÍDICO E BOTÃO FECHAR */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "1px solid #e2e8f0", paddingBottom: "10px" }}> {/* -> Alinhador horizontal compacto do cabeçalho do modal flutuante. */}
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <UserPlus size={14} strokeWidth={2.5} style={{ color: "#0f172a" }} /> {/* -> Injeta o componente vetorial de criação de usuário do Lucide no lugar do antigo emoji. */}
        ...
        <form onSubmit={tratarCadastroContato} style={{ display: "flex", gap: "12px", flexDirection: "column" }}> {/* -> Abre o formulário organizando a fiação de inputs empilhados verticalmente de forma densa. */}
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}> {/* -> Alinhador flexbox responsivo lado a lado para otimização de espaço em tela. */}
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1 }}> {/* -> Ocupa um terço da proporção horizontal focando o documento de identificação fiscal CPF. */}
              ...
              </label> {/* -> Rótulo em caixa alta indicando campo de documento rigidamente obrigatório. */}
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}> {/* -> Alinhador flexbox responsivo duplo para canais de acionamento e cobrança. */}
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1 }}> {/* -> Coluna flexível dedicada ao endereço eletrônico cadastral. */}
              ...
              </label> {/* -> Rótulo informativo cinza neutro para indicar campo opcional de contato. */}
          ...
          <div style={{ display: "flex", flexDirection: "column" }}> {/* -> Alinhador vertical em lote para classificação de dependência civil e papel do contato. */}
            ...
            </label> {/* -> Rótulo informativo cinza neutro identificador da caixa de seleção. */}
            <select 
              value={conTipo} 
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "12px", borderTop: "1px solid #e2e8f0", paddingTop: "12px" }}> {/* -> Adicionado traço de divisão sutil e padding superior para otimização visual sóbria del fechamento. */}
            <button type="button" onClick={aoFechar} style={{ background: "#ffffff", border: "1px solid #cbd5e1", padding: "6px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: "600", color: "#475569", transition: "background 0.15s ease" }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#f8fafc"} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "#ffffff"}>Cancelar</button> {/* -> Botão sóbrio de recusa voluntária que descarta as alterações e fecha o modal. */}
            <button type="submit" style={{ background: "#0f172a", color: "#ffffff", border: "none", padding: "6px 14px", borderRadius: "6px", fontWeight: "700", cursor: "pointer", fontSize: "12px", transition: "background 0.15s ease" }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#1e293b"} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "#0f172a"}>Vincular Representante</button> {/* -> Botão mestre de salvaguarda sólido Azul Escuro Profundo que submete o formulário. */}
          </div> {/* -> Encerra o agrupador de botões del rodapé. */}

        </form> {/* -> Encerra a tag estrutural del formulário de elos relacionais humanos. */}
      </div> {/* -> Encerra o cartão interno branco del modal flutuante. */}
