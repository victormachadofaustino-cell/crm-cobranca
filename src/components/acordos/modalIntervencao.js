export const modalIntervencao = { // Define e exporta o submódulo especialista em gerenciar as travas, baixas e estornos no mini-modal de propostas.
  renderizar(containerModal, fichaCobranca, callbackSalvarNuvem) { // Função de desenho que recebe o casulo invisível do HTML, a ficha do devedor e a comanda de gravação do Maestro.
    ...
    // CORRIGIDO: Adicionado encadeamento opcional estrito de segurança na validação e na clonagem por espalhamento, blindando a Vercel contra propostas nulas de clientes novos.
    let faturasProposta = Array.isArray(fichaCobranca.planoParcelas) && fichaCobranca.planoParcelas.length > 0 
      ...
      : (fichaCobranca.proposta?.parcelasSimuladas ? [...fichaCobranca.proposta?.parcelasSimuladas] : []); // Garante o retorno de uma lista vazia segura de fallback sem estourar o console do operador.
    ...
    const construirCorpoModal = () => { // Sub-rotina interna responsável por desenhar e atualizar os elementos do portal flutuante sem perder o sincronismo.
      ...
      containerModal.querySelectorAll('.mod-esteira-valor').forEach(inputV => {
        inputV.addEventListener('change', (ev) => {
          const indexAlvo = parseInt(ev.target.getAttribute('data-index')); // Localiza o ponteiro numérico da fatura editada.
          faturasProposta[indexAlvo].valor = parseFloat(ev.target.value) || 0; // Armazena a alteração monetária na linha correspondente.
        });
      });
      ...
      containerModal.querySelectorAll('.mod-esteira-data').forEach(inputD => {
        inputD.addEventListener('change', (ev) => {
          const indexAlvo = parseInt(ev.target.getAttribute('data-index')); // Localiza o ponteiro numérico da fatura editada.
          faturasProposta[indexAlvo].vencimento = ev.target.value; // Sincroniza a string estável de prazo com a nova data escolhida.
        });
      });
      ...
      containerModal.querySelectorAll('.btn-baixa-esteira-efetivar').forEach(btnB => {
        btnB.addEventListener('click', (ev) => {
          const indexAlvo = parseInt(ev.target.getAttribute('data-index')); // Identifica a fatura que está recebendo o dinheiro.
          const dataRelogio = new Date(); // Captura o relógio exato da operação de arrecadação da máquina local.
          
          faturasProposta[indexAlvo].pago = true; // Seta eletronicamente o recebimento como verdadeiro.
          faturasProposta[indexAlvo].status = 'pago'; // Padroniza o texto linear de conformidade do título.
      ...
      containerModal.querySelectorAll('.btn-reabrir-fatura-esteira').forEach(btnReab => {
        btnReab.addEventListener('click', (ev) => {
          const indexAlvo = parseInt(ev.target.getAttribute('data-index')); // Identifica a fatura que está recebendo o estorno.
          faturasProposta[indexAlvo].pago = false; // Estorna a flag de recebimento voltando o status para falso.
          faturasProposta[indexAlvo].status = 'a_vencer'; // Restaura a descrição clássica de pendente em aberto para o caixa.
          faturasProposta[indexAlvo].dataPagamento = ''; // Apaga permanentemente o carimbo cronológico antigo de liquidação.
          
          construirCorpoModal(); // Redesenha o modal liberando os inputs de valor e data para novas digitações operacionais.
        });
      });
      ...
      document.getElementById('btn-gravar-pop-esteira').addEventListener('click', async () => {
        ...
        const pacoteConsolidadoMudanca = {
          planoParcelas: faturasProposta, // Atualiza as linhas da esteira financeira da retaguarda de caixa.
          valorVencido: parseFloat(saldoRestanteEmAberto.toFixed(2)), // Reduz o saldo devedor principal do card no tabuleiro do Kanban.
          valor: parseFloat(saldoRestanteEmAberto.toFixed(2)), // Sincroniza o totalizador financeiro de topo de coluna cinza.
          proposta: {
            ...(fichaCobranca.proposta || {}), // Preserva parâmetros colaterais de fábrica estáveis.
            parcelasSimuladas: faturasProposta, // Sincroniza os inputs espelhados com a aba Proposta do prontuário.
